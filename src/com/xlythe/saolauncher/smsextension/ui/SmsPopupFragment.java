...
import com.xlythe.saolauncher.smsextension.preferences.ButtonListPreference;
...
public class SmsPopupFragment extends Fragment {
    ...
    private static final String EXTRA_PRIVACY_MODE = "com.xlythe.saolauncher.smsextension.privacy_mode";
    private static final String EXTRA_BUTTONS = "com.xlythe.saolauncher.smsextension.buttons";
    private static final String EXTRA_UNLOCK_BUTTON = "com.xlythe.saolauncher.smsextension.unlock_button";
    private static final String EXTRA_SHOW_BUTTONS = "com.xlythe.saolauncher.smsextension.show_buttons";
    ...
    public static SmsPopupFragment newInstance(SmsMmsMessage newMessage, int[] buttons, int privacyMode, boolean showUnlockButton, boolean showButtons) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if(message.isMms()) {
            // The ViewMMS button
            ((Button) v.findViewById(R.id.viewMmsButton)).setOnClickListener(new PopupButton(BUTTON_VIEW_MMS, buttonText));
        ...
        ((ImageButton) v.findViewById(R.id.viewButton)).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    private void setPrivacy(int newMode, boolean initial) {
        ...
        if((newMode != privacyMode || initial) && message != null) {
            final int viewPrivacy = message.isSms() ? VIEW_PRIVACY_SMS : VIEW_PRIVACY_MMS;
    ...
    private class FetchContactPhotoTask extends AsyncTask<Uri, Integer, Bitmap> {
        ...
        protected Bitmap doInBackground(Uri... params) {
            if(isAdded()) {
                if(BuildConfig.DEBUG) Log.v("Loading contact photo in background...");
        ...
        protected void onPostExecute(Bitmap photo) {
            if(BuildConfig.DEBUG) Log.v("Done loading contact photo");
            if(photo != null && viewReference != null) {
                final QuickContactBadge badge = viewReference.get();
                if(badge != null && isAdded()) {
                    TransitionDrawable mTd = new TransitionDrawable(new Drawable[] { getResources().getDrawable(R.drawable.ic_contact_picture),
    ...
    private class PopupButton implements OnClickListener {
        ...
        public PopupButton(int id, String[] buttonTextArray) {
            buttonId = id;
            isReplyButton = false;
            if(buttonId == ButtonListPreference.BUTTON_REPLY || buttonId == ButtonListPreference.BUTTON_QUICKREPLY
